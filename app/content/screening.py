...
# 품사 이름은 `lexicon.POS_KO_ALTERNATION` 에서 받아 온다. 여기 `(명사|동사|형용사|부사)`
# 를 박아 두었더니 **순우리말로 쓴 단정이 통째로 지나갔다** — "'name'은 이름씨로만
# 쓰여요" 가 아무 지적도 안 받았다. 이 자료는 표기를 한자어로 통일했지만, 검사기가
# 표기 하나에 매여 있으면 누군가 다시 순우리말로 쓰는 순간 구멍이 다시 열린다.
_POS = lexicon.POS_KO_ALTERNATION
_POS_ONLY_CLAIM = re.compile(
    _QUOTE + r"([A-Za-z][A-Za-z\-]{1,24})" + _QUOTE + rf"\s*(?:은|는)\s*({_POS})로만"
)

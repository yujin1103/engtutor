...
def unused_forms() -> list[str]:
    """형태 표에 있는데 **어느 틀도 안 부르는** 동사.

    한쪽 방향만 막으면 반대쪽이 조용히 쌓인다. `rules()` 는 "틀이 부르는데 표에
    없는 동사" 를 시작할 때 잡지만, 그 반대는 아무 데도 안 걸린다 — 틀에서 동사를
    빼면 표의 줄은 남고, 남은 줄은 사람이 검수한 것처럼 보인다.

    시작을 막지는 않는다. 죽은 줄이 학습자에게 나가지는 않고, 규칙을 하나 더
    만들 때 쓸 재료일 수도 있다. 대신 시험이 개수를 세어 늘어나는 것을 알린다.
    """
    used = {v for rule in rules().values() for f in rule.frames for v in f.verbs}
    return sorted(set(verb_forms()) - used)

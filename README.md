...
`app/tutor/grammar_rules/verb_forms.yaml` 에 동사 87개의 모양을 손으로 골라 넣었다
...
1. **id 앞 8자가 곧 보기 순서였다.** `hexdigest()[:8]` 과 `digest()[0:4]` 는 같은
   ...
   `order.index(0)` 이 답의 자리다. **151문제가 전부 뚫렸다** — 서버에 요청 한 번
   안 보내고.
...
그래서 둘을 갈랐다. **id 는 정답과 아무 관계 없는 자리 번호**(`to_infinitive#0007`)로
두어 해시해 볼 재료 자체를 없앴고, **보기 순서는 응답 밖의 값**(프로세스마다 새로
뽑는 소금)을 섞어 만든다. 프로세스가 다시 뜨면 순서가 바뀌는데, 그건 학습자에게
보이지 않는 상태라 상관없다 — 한 번 푸는 동안은 고정이다.
...
자유 입력 칸(`said`)은 들어올 때 다듬는다. 5MB 를 보내면 5MB 가 그대로 돌아오던
것도 같이 막았다.

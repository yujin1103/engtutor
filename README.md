...
- **id** — 처음에는 `to_infinitive:Please remember to ___ the invoice.:send` 로 지었다.
  읽으면 답이 보인다. **응답에서 정답을 뺀 것이 아무 소용이 없어진다.** 지금은
  같은 재료로 매번 같은 값이 나오되 되읽을 수 없는 sha256 앞 12자다.
- **보기 순서** — 서버가 문제 id 를 씨앗으로 굳혀 보낸다. 화면에서 섞으면 새로고침마다
  답의 자리가 달라져 서버가 채점한 것과 학습자가 본 것이 어긋난다. 매번 무작위로
  섞어도 안 된다 — 같은 문제를 다시 풀 때 답이 옮겨 다닌다.

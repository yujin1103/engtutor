...
### 영어를 아예 모르는 사람에게는 단서가 하나도 없었다

빈칸이 주는 단서는 넷이다 — 낱말 뜻, 문장 해석, 문형, 품사. **넷 다 한국어다.**

알파벳을 못 읽는 사람은 뜻을 다 알고도 첫 글자를 못 적는다. "펜 좀 빌려도 될까요?"
를 읽고 "빌리다" 라는 뜻까지 보고도 `b` 를 못 쓴다. 그 사람에게 빈칸은 문제가 아니라
**벽**이고, 벽 앞에서는 연습이 시작되지 않는다.

그래서 철자 단서를 **한 걸음씩** 편다.

| 걸음 | 보이는 것 |
|---|---|
| 1 | `_ _ _ _ _ _`  일곱 글자예요. |
| 2 | `b _ _ _ _ _`  `'b'` 로 시작해요. |
| 3 | `b o r _ _ _`  앞 세 글자는 `'bor'` 예요. |

**마지막 걸음까지 봐도 최소 한 글자는 밑줄로 남는다.** 다 드러내면 연습이 아니라
베껴 쓰기가 된다. 그 한 글자를 학습자가 적어야 과제가 성립한다.

짧은 답에는 적게 준다. 두 글자짜리(`am`, `it`)는 글자 수 하나뿐이다 — 거기서 첫
글자를 주면 남는 것이 없다. 한 글자짜리(`a`)는 아예 안 준다. 글자 수가 곧 정답이다.

**자동으로 펴지 않는다.** 저절로 열리면 문장을 읽기 전에 답의 모양이 먼저 눈에
들어온다. 처음 한 번은 문장과 뜻만 보고 겨눠 보게 둔다.

**대소문자는 낮춘다.** `It is a book.` 의 답은 `It` 이지만 단서는 `_ _` 로 준다.
문장 맨 앞이라 대문자인 것을 그대로 보여 주면, 자리 때문에 생긴 모양을 철자로
가르치게 된다. 채점은 어차피 대소문자를 가리지 않는다.

**아포스트로피는 처음부터 보여 준다.** `don't` 는 `_ _ _ ' _` 다. 그건 철자가 아니라
짜임이라 가리면 오히려 어렵다. 글자 수도 다섯이 아니라 **네 글자**로 센다.

#### 조사 하나가 이 기능의 품질이다

`'n' 으로 시작해요` 인데 `'a' 로 시작해요` 다. 영어 낱말 뒤에 붙는 한국어 조사는
**글자가 아니라 그 글자를 우리말로 읽은 소리**가 정한다.

처음에 `f`·`h`·`s`·`x` 도 받침이 있다고 보고 `'f' 으로` 라고 썼는데 틀렸다.
에프·에이치·에스·엑스는 프·치·스·스로 끝나 받침이 없다. 받침이 남는 것은
**엘·엠·엔·알 넷뿐**이고, 그중 `l`·`r`은 받침이 ㄹ이라 `으로`를 안 쓴다 —
`엘로`이지 `엘으로`가 아니다. 그래서 조사 둘이 서로 다른 집합을 본다.
...
영어를 아예 모르는 사람에게 보여 줄 문구라, 조사가 어긋나면 그게 제일 먼저 눈에 띈다.
...
## 토익 문법 문제 — 규칙이 정답을 정하니까 LLM 이 필요 없다

토익 Part 5 형 4지선다. 보기 넷이 **전부 같은 낱말의 다른 모양**이다.

```
Please remember to ____ the invoice.
① send   ② sending   ③ sender   ④ sent
```

정답 근거가 "to 뒤에는 동사원형" 이라는 **규칙**이라, 출제도 채점도 결정론적이다.
빈칸 연습장과 나란히 놓으면 묻는 것이 다르다 — 저쪽은 **뜻**을 묻고(한국어 해석이
단서다), 이쪽은 **형태**를 묻는다. 뜻은 이미 보기 넷이 알려 준 셈이다.

그래서 `cloze.py` 의 함수를 하나도 재사용하지 않는다. 지우는 자리가 표제어가 아니고,
채점이 자유 입력이 아니라 보기 비교이며, 무엇보다 **`pos_hint`("여기엔 동사가
들어가요")가 이 문제에서는 정답 그 자체**라 함께 내보내면 문제가 사라진다.

### 문장을 손으로 쓴 이유 — `to` 가 부정사인지 전치사인지 판정할 수가 없다

처음에는 토익 예문에서 캐려 했다. 2,252개에서 `to + 낱말` 이 373건 나온다. 그런데
그 대부분이 **전치사** `to` 다.

| | 개수 |
|---|---|
| 예문에 `to + 낱말` | 373 |
| 그중 뒤가 **동사로만** 쓰이는 낱말 (부정사가 확실) | 109 |
| 그중 `need to` 한 틀 | **84** |

`to work`(일하러/직장으로), `to our company`, `to tomorrow` 를 규칙으로 갈라낼 방법이
이 저장소에 없다. 확실한 것만 남기면 109건인데 그중 84건이 `need to` 라, 문제가 죄다
"We need to ______" 로 나온다.

그래서 문장 틀을 손으로 쓴다(`app/tutor/grammar_rules/to_infinitive.yaml`, 24개).
**틀을 사람이 쓰면 그 `to` 가 부정사라는 것이 작성 시점에 보증되고, 판정할 필요가
사라진다.** 각 틀의 `verbs` 는 그 문장에 넣어 말이 되는 동사만 적은 것이고 그 목록이
곧 검수다. 24개 틀 × 동사 = **134문제**.

### 낱말의 모양도 데이터다 — WordNet 은 철자를 검사하지 못한다

오답 보기는 `sending`·`sender`·`sent` 처럼 같은 낱말의 다른 모양이다. 이걸 코드로
만들려고 했고, **안전하게는 안 된다.**

사용자가 처음 든 예시부터 그 함정을 담고 있었다 — 보기 ④ 가 `sendable` 이었는데
**그런 낱말은 없다.** 이 프로젝트가 계속 싸워 온 지어낸 파생어다.

사전으로 거르면 될 것 같지만, 검사기 셋이 다 통과시킨다.

| 지어낸 형태 | `morphy(v)` | `lexicon.known` | WordNet 색인 원본 |
|---|---|---|---|
| `builded` | `'build'` | `True` | 없음 |
| `attachs` | `'attach'` | `True` | 없음 |
| `sended` | `'send'` | `True` | 없음 |
| `announceing` | `'announce'` | `True` | 없음 |

접미사를 떼어 원형에 닿기만 하면 통과시키기 때문이다 — 그게 **원형 찾기**의 일이지
철자 검사의 일이 아니다. 색인 원본(`all_lemma_names`, 147,306개)은 반대로 너무 좁아
`announcing`·`canceling` 같은 멀쩡한 형태가 표제어에 없다. 완전한 표면형 사전이 없다.

게다가 `-ing` 의 자음 겹치기는 **강세**에 달려 있어 규칙 자체가 없다 —
`prefer→preferring` 인데 `offer→offering` 이고, `open→opening` 이지 `openning` 이 아니다.

그래서 낱말 콘텐츠와 같은 원칙으로 간다. **미리 만들고, 사람이 보고, 서빙은 조회.**
`app/tutor/grammar_rules/verb_forms.yaml` 에 동사 90개의 모양을 손으로 골라 넣었다
(`extensor`(해부학 용어)·`solvent`(다른 뜻)·`annunciatory`·`obtention` 같은 것을
걷어냈다). 지어낸 철자가 학습자 앞에 놓일 길이 아예 없다.

### 정답이 새는 길이 셋이다

응답 필드, 문제 id, 보기 순서. 셋 다 막아야 하고 셋 다 실수하기 쉽다.

- **필드** — `GET /grammar` 응답에 `answer` 가 없다. 채점은 `POST /grammar/answer` 가 한다.
- **id** — 처음에는 `to_infinitive:Please remember to ___ the invoice.:send` 로 지었다.
  읽으면 답이 보인다. **응답에서 정답을 뺀 것이 아무 소용이 없어진다.**
- **보기 순서** — 서버가 굳혀 보낸다. 화면에서 섞으면 새로고침마다 답의 자리가
  달라져 서버가 채점한 것과 학습자가 본 것이 어긋난다. 매번 무작위로 섞어도 안
  된다 — 같은 문제를 다시 풀 때 답이 옮겨 다닌다.

보기가 **어느 모양인지도 채점 전에는 보내지 않는다.** `sending` 옆에 '동명사' 라고
적어 두면 규칙을 아는 학습자가 문장을 안 읽고 고른다.

#### 해시로 지어도 샌다 — 후보가 넷뿐이면 넷을 쳐 보면 된다

두 번째 판은 id 를 `sha256(f"{rule}:{frame}:{verb}")[:12]` 로 지었다. 되읽을 수
없으니 됐다고 봤는데, **되읽기 어려운 것과 맞혀 보기 어려운 것은 다른 성질이다.**

적대적 검토가 두 길을 찾아냈다.

1. **id 앞 8자가 곧 보기 순서였다.** `hexdigest()[:8]` 과 `digest()[0:4]` 는 같은
   바이트인데, 자리바꿈이 `digest[0..3]` 을 쓰고 있었다. 정답은 늘 섞기 전 0번이라
   `order.index(0)` 이 답의 자리다. **134문제가 전부 뚫렸다** — 서버에 요청 한 번
   안 보내고.
2. **id 자체가 정답에 대한 검증 가능한 커밋먼트였다.** 씨앗 재료가 전부 응답에서
   복원된다 — 규칙 이름은 그대로 있고, 틀은 문장의 빈칸을 되돌리면 되고, 동사는
   보기 넷 중 하나다. 네 번 해시해 맞는 것을 고르면 끝이다. 1번을 고쳐도 남는다.

그래서 둘을 갈랐다.

**id 는 정답을 뺀 내용**으로 짓는다 — `sha256(규칙 : 틀 문장 : **정렬한** 보기 넷)` 의
앞 열두 자다. 응답만 보고도 똑같이 계산할 수 있는데 그래도 아무것도 안 샌다.
정렬해 넣으면 넷 중 어느 것이 답인지가 씨앗에 안 들어가기 때문이다.

자리 번호(`to_infinitive#0007`)를 한 번 거쳤다가 되돌렸다. 맞혀 볼 재료는 없앴는데
id 가 내용이 아니라 **자리**를 가리켜서, 틀 하나를 지우면 그 뒤가 전부 한 칸씩
조용히 민다 — 404 도 안 난다. 검수하는 동안 146→141→134 로 바뀌면서 146개 중
121개가 다른 문제를 가리켰다.

**보기 순서는 응답 밖의 값**(프로세스마다 새로 뽑는 소금)을 섞어 만든다. 프로세스가
다시 뜨면 순서가 바뀌는데, 그건 학습자에게 보이지 않는 상태라 상관없다.

#### 거절할 값에 500 으로 답하면 안 된다

같은 검토가 500 을 찾았다. 짝이 없는 서로게이트 문자를 보내면 **검증은 제대로
거절해 놓고 그 거절 사유를 JSON 으로 옮기다** `UnicodeEncodeError` 가 난다.
`/cloze/answer` 의 `said`·`word` 도 같았다. 거절당할 값을 보냈는데 "서버가 부서졌다"
는 답이 돌아오면 진짜 고장과 구별이 안 된다.

오류를 **만들다** 난 고장이라 필드마다 막아서는 안 잡힌다. 그래서 나가는 문 둘
(`RequestValidationError`·`HTTPException`)에서 옮길 수 없는 글자를 바꿔 내보내고,
자유 입력 칸(`said`)은 들어올 때 다듬는다.

#### 되비추기도 필드 제한으로는 못 막았다

같은 자리에 5MB 를 보내면 5MB 가 그대로 돌아왔다. 처음에는 `max_length` 를 붙여
막았다고 적었는데 **거짓이었다** — 재검수가 다시 재 보니 그대로였다.

```
chat message 5MB → 422 인데 본문 5,000,152B
```

`max_length` 는 값을 **거절**하지만, pydantic 이 거절한 그 값을
`errors()[0]["input"]` 에 통째로 담고 처리기가 그걸 실어 보낸다. 거절과 되싣기는
다른 일이라, 자르는 것은 되싣는 자리에서 해야 한다(`_utf8_safe`, 200자).
지금은 300KB 를 보내도 응답이 360B 다. 시범으로 바깥에 열어 두는 앱이라
무제한 응답을 남겨 둘 자리가 아니다.

### 틀을 돌아가며 낸다

`items_of` 가 틀 하나를 끝내고 다음으로 가면 처음 열 문제가 전부 같은 문장이다.
문제는 다르지만 화면은 같아 보이고, 그러면 문장을 안 읽고 보기만 보게 된다 —
이 유형에서 그건 연습을 통째로 무의미하게 만든다. 그래서 틀을 돌아가며 뽑는다.
무작위로 섞지는 않는다. `offset` 으로 다음 쪽을 받는 화면이 문제를 건너뛰거나
두 번 받게 되기 때문이다.

 ...
 * 주소창은 그대로 두고 상태만 태운다.
 *
 * **새로고침하면 보던 화면이 그대로 다시 뜬다.** 여기 오래 "처음 화면으로
 * 돌아간다" 고 적혀 있었는데 사실이 아니다 — 브라우저는 방문 기록 한 칸에 붙은
 * state 를 새로고침 뒤에도 돌려주고, 아래 `useState` 초기값이 그것을 읽는다.
 * 헤드리스 크롬으로 화면을 찍다 드러났다(문법 문제 화면에서 새로고침했더니
 * 홈이 아니라 문법 문제가 다시 떴다).
 *
 * 폰 앱으로 감싸 놓고 보면 나쁘지 않은 동작이지만, `chat` 으로 돌아오면 주고받은
 * 말이 비어 있다 — 그것들은 React 상태에만 있다. **더 나쁜 것은 세션 손잡이까지
 * 같이 사라지던 것**이었다: 되살아난 화면에 '끝내기' 가 안 그려지고, 세션을 닫는
 * 것은 리포트뿐이라 서버 세션이 ended_at=NULL 로 영영 남았다.
 *
 * 그래서 `chat` 라우트가 `sessionId` 를 함께 태운다(아래 유니온). history.state 는
 * 새로고침을 견디므로 그 한 칸으로 세션이 살아남고, 이어 보내기와 리포트가 그대로
 * 된다 — report 라우트가 처음부터 그렇게 해 온 것과 같은 처방이다. 말풍선까지
 * 되살리는 것은 아직 안 했다(서버가 해석·say_en 을 안 들고 있어 따로 볼 일이다).
 */
...
export type Route =
   ...
   * 대화 화면.
   *
   * `sessionId` 는 서버가 첫 답과 함께 알려 주면 그때 `replace` 로 실어 둔다.
   * **여기 태워 두지 않으면 새로고침 한 번에 세션이 영영 미아가 된다** — 화면은
   * history.state 덕에 되살아나는데 세션 손잡이는 React 상태에만 있어서, 되살아난
   * 화면에는 '끝내기' 가 안 그려지고(그 버튼은 sessionId 가 있어야 나온다) 리포트로
   * 가는 길이 사라진다. 세션을 닫는 것은 리포트뿐이라, 서버 쪽 세션은 ended_at 이
   * NULL 인 채 남는다. report 라우트가 처음부터 sessionId 를 들고 다녀서 새로고침에
   * 안 깨지는 것과 같은 처방이다.
   */
  | { name: "chat"; scenarioId: string; sessionId?: string }

 ...
 *
 *     빈칸 연습장에 있는 '다시 풀기' 를 여기에는 일부러 안 뒀다. 거기는 자유
 *     입력이라 다시 쓰는 데 뜻이 있지만, 여기는 보기가 넷이고 판정이 나면 답이
 *     이미 하이라이트돼 있다 — 그 상태로 다시 고르게 하면 남은 셋을 눌러 보는
 *     것이 최적 전략이 되어 정답률이 아니라 클릭 수를 재는 문제가 된다. 이
 *     화면에서 다시 만나야 할 것은 **같은 화면의 재시도가 아니라 나중에 다시
 *     나오는 그 문제**다(문제 id 를 자리 번호가 아니라 내용 해시로 잡아 둔 것이
 *     그 때문이다 — `app/tutor/grammar.py` 참고). 그 오답 노트는 아직 없다.
